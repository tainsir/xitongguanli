...
class llllMiddleware(MiddlewareMixin):
    """
    检查用户的url请求是否是其权限范围内
    """

    def process_request(self, request):
        request_url = request.path_info  # 获取用户实际请求路由url
        permission_url = request.session.get(settings.SESSION_PERMISSION_URL_KEY)  # 获取用户权限url列表
